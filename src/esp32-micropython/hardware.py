...
servo = Servo(
    pin=config.PIN_SERVO,
    angulo_inicial=config.SERVO_CENTRO,
)
...
NOMBRES_TOF = ("frontal", "izquierdo", "derecho")
...
tofs = {nombre: None for nombre in NOMBRES_TOF}
...
def _crear_tof(nombre):
    ...
    if direccion not in i2c.scan():
        raise OSError(
            "ToF {} no responde en 0x{:02X}".format(nombre, direccion)
        )
    sensor.start()
    tofs[nombre] = sensor
    return sensor
